...
class InclinedPlaneScene(Scene):
    def construct(self):
        ...
        for v in vectors:
            ...
            label = MathTex(v_label, color=v_color).scale(0.8)
...
class Collision1DScene(Scene):
    def construct(self):
        ...
        label1 = Tex(f"m1={m1}kg").scale(0.5).next_to(balok1, DOWN)
        label2 = Tex(f"m2={m2}kg").scale(0.5).next_to(balok2, DOWN)
        self.play(Write(label1), Write(label2))
        v1_arrow = Arrow(ORIGIN, v1_awal * RIGHT * 0.5, color=GREEN, buff=0)
        v1_arrow.next_to(balok1, UP, buff=0.2)
        v1_label = MathTex(f"v_1={v1_awal:.1f}").scale(0.5).next_to(v1_arrow, UP)
        v2_arrow = Arrow(ORIGIN, v2_awal * RIGHT * 0.5, color=RED, buff=0)
        v2_arrow.next_to(balok2, UP, buff=0.2)
        v2_label = MathTex(f"v_2={v2_awal:.1f}").scale(0.5).next_to(v2_arrow, UP)
        ...
        v1f_label = MathTex(f"v_1'={v1_akhir:.1f}").scale(0.5).next_to(v1f_arrow, UP)
        v2f_arrow = Arrow(ORIGIN, v2_akhir * RIGHT * 0.5, color=RED, buff=0)
        v2f_arrow.next_to(balok2, UP, buff=0.2)
        v2f_label = MathTex(f"v_2'={v2_akhir:.1f}").scale(0.5).next_to(v2f_arrow, UP)
