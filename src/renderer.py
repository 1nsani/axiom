...
class InclinedPlaneScene(MovingCameraScene):
    def construct(self):
        ...
        panj = 1.5  # default panjang panah
        any_floor = False  # catatan visual jujur
...
class AtwoodMachineScene(MovingCameraScene):
    def construct(self):
        ...
        from renderer_registry import resolve_direction, SUPPORTED_COLORS, scale_vector_length
        bx, by = RIGHT, UP
        # Kumpulkan magnitudo untuk scaling
        all_magnitudes = []
        for vec in vectors:
        ...
        for vec in vectors:
            ...
            if mag is not None and all_magnitudes:
                panj = scale_vector_length(mag, all_magnitudes)
            else:
                panj = 1.8
            if panj == 0.4 and mag is not None and mag > 0.1:
                any_floor = True

            if "up" in logic:
                arrow = Arrow(ORIGIN, panj * dir_vec, buff=0, color=col, stroke_width=5)
