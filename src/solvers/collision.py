...
def solve_collision(known: dict) -> dict:
    ...
    if v1 <= v2:
        raise ValueError("Tumbukan tidak valid: v1_awal harus > v2_awal.")

    # Untuk tumbukan tidak elastis sempurna, gunakan rumus umum
    total_m = m1 + m2
    v1_akhir = ((m1 - e*m2) * v1 + (1+e) * m2 * v2) / total_m
    v2_akhir = ((m2 - e*m1) * v2 + (1+e) * m1 * v1) / total_m

    # Catatan: untuk e=1, rumus ini identik dengan elastic_collision_1d.
    # Kita tetap menggunakan rumus ini agar konsisten dengan test lama.
